...
async function sendTransaction(){
	...
	contract.methods.transfer(sentAddress.value,sentAmount.value).send({ from : window.userWalletAccount })
	.then((res) => {
		console.log(res)
		transactionMessage.innerText = 'Transaction success ! Your transaction hash is : ' + res.transactionHash
		transactionMessage.href = "https://kovan.etherscan.io/tx/"+res.transactionHash
		debugger
	})
	.catch((err) => {
		alert(err.message)
...
document.addEventListener('DOMContentLoaded', async function (){
	...
	.then( (accounts) => {
		...
		contract.methods.balanceOf(window.userWalletAccount).call()
		.then((bal) => {
			balance.style.display = 'block'
			balance.innerText = 'Your account balance : ' + bal.toString()
		})
		.catch((error) => {
			console.error(error)
		})
